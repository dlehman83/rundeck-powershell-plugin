...
elseif ($ENV:RD_EXEC_COMMAND -like "/tmp/*") {
    ...
    if ($ENV:RD_NODE_DESCRIPTION -like "Rundeck server node") {
        ...
        Invoke-Command -ComputerName $ENV:RD_NODE_HOSTNAME -Credential $Credentials -Filepath "$($path).ps1" -Authentication $ENV:RD_CONFIG_AUTHTYPE
    }
}
...
#region command-handling
    ...
    if ($ENV:RD_NODE_DESCRIPTION -like "Rundeck server node") {
        ...
        Invoke-Command -ComputerName $ENV:RD_NODE_HOSTNAME -Credential $Credentials -Command $scriptblock -Authentication $ENV:RD_CONFIG_AUTHTYPE
    }
}
#endregion